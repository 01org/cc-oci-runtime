...
handle_state_pod_section(GNode* node, struct handler_data* data) {
	...
	if (g_strcmp0(node->data, "sandbox") == 0) {
		pod->sandbox = *((gboolean *)(node->children->data));
		(*(data->subelements_count))++;
	} else if (g_strcmp0(node->data, "sandbox_id") == 0) {
		pod->sandbox_id =
			g_strdup ((gchar *)node->children->data);
		(*(data->subelements_count))++;
...
cc_oci_state_file_read (const char *file)
{
	...
	if (state) {
		...
		state->pod = g_malloc0 (sizeof(struct cc_pod));
		if (! state->pod) {
			g_free (state->proxy);
			g_free (state->vm);
			g_free (state);
...
cc_oci_state_free (struct oci_state *state)
{
	...
	if (state->pod) {
		g_free_if_set (state->pod->sandbox_id);
		...
		const char *created_timestamp)
{
	...
	if (config->pod != NULL) {
		...
		json_object_set_string_member (pod, "sandbox_id",
					       config->pod->sandbox_id ?
					       config->pod->sandbox_id : "");
