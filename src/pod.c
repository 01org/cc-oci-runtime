...
#define CC_POD_OCID_SANDBOX        "infrastructure"
#define CC_POD_OCID_CONTAINER      "container"

#define CC_POD_OCID_SANDBOX_ID   "ocid/sandbox_id"
...
cc_pod_handle_annotations(struct cc_oci_config *config, struct oci_cfg_annotation *annotation)
{
	...
	if (strncmp(annotation->key, CC_POD_OCID_ANNOTATION_NAMESPACE,
		    CC_POD_OCID_ANNOTATION_NAMESPACE_SIZE) != 0) {
		return 0;
	}
	...
	} else if (g_strcmp0(annotation->key, CC_POD_OCID_SANDBOX_ID) == 0) {
		config->pod->sandbox_id = g_strdup(annotation->value);
